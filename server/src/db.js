...
const SEED_ROADMAP_PATH = process.env.SEED_ROADMAP_PATH ||
  resolve(__dirname, '../../assets/data/roadmap.json');
...
const initMigration = readFileSync(resolve(__dirname, 'migrations/001_init.sql'), 'utf-8');
db.exec(initMigration);
const roadmapMigration = readFileSync(resolve(__dirname, 'migrations/002_roadmap.sql'), 'utf-8');
db.exec(roadmapMigration);

// Seed: ensure a "Système" user exists, an initial tree revision and an initial roadmap revision are present.
function seed() {
  ...
  const treeHead = db.prepare('SELECT id FROM revisions ORDER BY id DESC LIMIT 1').get();
  if (!treeHead) {
    let seedTree;
    try {
  ...
  const roadmapHead = db.prepare('SELECT id FROM roadmap_revisions ORDER BY id DESC LIMIT 1').get();
  if (!roadmapHead) {
    let seedRoadmap;
    try {
      seedRoadmap = JSON.parse(readFileSync(SEED_ROADMAP_PATH, 'utf-8'));
    } catch {
      seedRoadmap = { meta: {}, items: [] };
    }
    db.prepare(`
      INSERT INTO roadmap_revisions (parent_id, data_json, author_id, message)
      VALUES (NULL, ?, ?, ?)
    `).run(JSON.stringify(seedRoadmap), sysId, 'Initialisation depuis roadmap.json');
  }
}
...
export function getRoadmapHeadRevision() {
  return db.prepare(`
